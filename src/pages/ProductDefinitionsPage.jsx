...
import { Container, Box, Flex, Heading, TextField, Select, Text, Button, Card, Badge, IconButton, Dialog } from '@radix-ui/themes';
import { Search, Plus, GitMerge, Trash2, Edit } from 'lucide-react';
                ...
                {filteredDefinitions.map(def => (
                    <Card key={def.id} className="hover:shadow-md transition-shadow border-l-4 border-l-teal-500 flex flex-col">
                        <Box className="flex-grow cursor-pointer p-3" onClick={() => navigate(`/product-definition/${def.id}`)}>
                            <Flex direction="column" gap="2">
                                ...
                                <Text size="1" color="gray">
                                    Created: {def.createdAt?.toDate ? def.createdAt.toDate().toLocaleDateString() : 'Just now'}
                                </Text>
                                <Flex gap="2" mt="2">
                        ...
                        <Flex justify="end" p="2" className="border-t border-gray-100">
                             <IconButton size="1" variant="ghost" color="red" onClick={(e) => handleDelete(def.id, e)}>
                                <Trash2 size={14} />
                            </IconButton>
                        </Flex>
                    </Card>
