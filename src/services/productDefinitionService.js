...
const COLLECTION_NAME = 'productDefinitions';
 ...
 */
export const createProductDefinition = async (userId, title = "New Product Definition") => {
  ...
  const newDoc = {
    ...
    linkedPyramidId: null, // Legacy field, kept for backward compatibility
    contextSources: [], // New field: Array of { type: 'pyramid' | 'productDefinition' | 'contextDocument', id: string, title: string }
    ...
    data: {
      ...
      "1": { id: "1", label: "1. Problem & Goals", parent: "root", children: ["1-1", "1-2", "1-3"] },
        "1-1": { id: "1-1", label: "Problem Statement", parent: "1", children: ["1-1-1", "1-1-2", "1-1-3"] },
 ...
 */
export const getProductDefinition = async (id) => {
 ...
 */
export const updateProductDefinitionNode = async (definitionId, nodeId, newData) => {
  const docRef = doc(db, COLLECTION_NAME, definitionId);
  const docSnap = await getDoc(docRef);
...
/**
 * Update linked pyramid
 */
export const linkPyramidToDefinition = async (definitionId, pyramidId) => {
  const docRef = doc(db, COLLECTION_NAME, definitionId);
  await updateDoc(docRef, {
    linkedPyramidId: pyramidId,
    lastModified: serverTimestamp()
  });
};
 ...
 */
export const updateNodeDescription = async (definitionId, nodeId, description) => {
  await updateProductDefinitionNode(definitionId, nodeId, { description });
};

/**
 * Delete a product definition
 */
export const deleteProductDefinition = async (id) => {
  const docRef = doc(db, COLLECTION_NAME, id);
 ...
 */
export const getUserProductDefinitions = async (userId) => {
  if (!userId) return [];
  const q = query(
 ...
 */
export const updateContextSources = async (definitionId, sources) => {
  const docRef = doc(db, COLLECTION_NAME, definitionId);
  await updateDoc(docRef, {
